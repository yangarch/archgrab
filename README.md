...
| 계층 | 내용 |
|---|---|
| 추출 | 쿠키 없으면 `yt-dlp` 단독(익명 가능), 쿠키 있으면 `gallery-dl` 우선. 어댑터로 감쌈 |
...
**공개 게시글·릴스·캐러셀은 쿠키 없이 받을 수 있다.** 인스타그램에는 비로그인
전용 GraphQL 경로가 있고(`PolarisLoggedOutDesktopWWWPostRootContentQuery`,
응답 필드 `if_not_gated_logged_out`), yt-dlp 가 이걸 쓴다.

단 조건이 하나 있다: **브라우저 TLS 지문 위장**이 되어야 한다. `curl_cffi` 가
그 역할을 하고, 그래서 선택 의존성이 아니라 정식 의존성으로 박아뒀다. 이게 없으면
yt-dlp 의 위장 타겟이 0개가 되어 익명 경로가 **조용히** 비활성되고, 공개
게시글까지 "로그인 필요"처럼 실패한다. `/api/engines` 의 `anonymous_instagram`
필드로 현재 상태를 확인할 수 있다.

gallery-dl 은 익명 인스타 접근이 안 된다 — `browser=firefox`·`browser=chrome`·
`api=graphql` 모두 로그인 페이지로 리다이렉트된다. 그래서 쿠키가 없을 때는 아예
호출하지 않는다. 쿠키가 있으면 이미지·캐러셀 메타데이터가 더 정확해서 우선한다.
...
- [x] **M1** 인스타그램 — 실제 게시글로 종단 검증 완료(쿠키 없이 resolve → 다운로드 →
      원본 확인). 영상+음성 트랙이 모두 들어오고 재인코딩 흔적이 없는 것까지 `ffprobe`
      로 확인. **이미지 전용 게시글은 아직 미검증** — yt-dlp 가 이미지를 `formats` 가
      아니라 `thumbnails`(`image_versions2` 후보)에 담아서, 최대 후보를 원본으로 집도록
      구현했지만 실제 이미지 게시글로 확인하지 못했다.
- [ ] **M2** X
- [ ] **M3** 유튜브
- [ ] **M4** 외부 노출 · 히스토리
